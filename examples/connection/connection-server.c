...
static void
print_error(const char *func, const int ret)
{
	int result = 0;
...
main(int argc, char *argv[])
{
	...
	if (ret) {
		...
		const char *eventstr = rpma_utils_conn_event_2str(conn_event);
		fprintf(stderr, "rpma_conn_next_event return an unexptected event: %s\n", eventstr);
		goto err_conn_delete;
	}
